...
interface Props {
  ...
  autoGenerate?: boolean; // when true, fire generate immediately on mount (coming from planner)
}

export function GroceryClient({ weekStart: initialWeekStart, autoGenerate = false }: Props) {
  ...
  // Start in generating state when auto-triggered so the skeleton persists until the
  // list is ready — prevents a flash of the "No grocery list yet" empty state.
  ...
  // Ref guard: ensures auto-generate fires exactly once per mount even if deps change
  ...
  // Auto-trigger when arriving from the planner via ?generate=true
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        ...
        {/* Show skeleton while loading OR while auto-generating (suppresses empty-state flash) */}
        {loading || (generating && !list) ? (
